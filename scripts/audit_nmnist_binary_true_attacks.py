...
def main() -> None:
    ...
    if audit_batch != 64:
        raise RuntimeError("AUDIT_BATCH_SIZE must be 64")
    with torch.no_grad():
        for start in range(0, len(expected_ids), audit_batch):
            end = min(start + audit_batch, len(expected_ids))
            clean_tensor = torch.from_numpy(np.array(clean_all[start:end], dtype=np.float32, copy=True)).to(device)
            adv_tensor = torch.from_numpy(np.array(reconstructed[start:end], dtype=np.float32, copy=True)).to(device)
            clean_batch = model(clean_tensor).argmax(1).cpu().tolist()
            adv_batch = model(adv_tensor).argmax(1).cpu().tolist()
            for local, i in enumerate(range(start, end)):
                clean_prediction = int(clean_batch[local])
                adv_prediction = int(adv_batch[local])
