...
def main():
    ...
    if BATCH != 64 or AUDIT_BATCH != 64: raise RuntimeError("ATTACK_BATCH_SIZE and AUDIT_BATCH_SIZE must both be 64")
    ...
    for seed in seeds:
        ...
        for kind,budget in conditions:
            run_condition(seed,model,checkpoint,manifest,manifest_path,cache,cache_path,kind,budget,device);aggregate()
    aggregate()
